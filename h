...
echo $PWD
echo $ls
	mv "$current_rel_backup/*" $PWD
	mv "$prev_rel_backup/*" $PWD
...
update_symlinks(){
	...
	# echo "hi ajay"
	# echo $current_rel
	create_symlink $tempReleaseD $tempPwd $3 $temp_release $tempDirTest
...
kashish(){
	...
	then
		...
		update_symlinks $2 $PWD $next_release_folder_name $3
		if [ $? == 0 ]
		then
